...
void EncryptionHelper::getPmk(const std::string& password, const uint8_t suite, const std::string& ssid, uint8_t* pmk)
{
	if (pmk == nullptr)
...
void EncryptionHelper::getPtk(uint8_t* ptk, const uint8_t* pmk, const uint8_t* data, const uint8_t suite)
{
	...
	while (offset < PTK_SIZE)
	{
		...
		const size_t copySize = std::min<size_t>(PTK_SIZE - offset, digestSize);
...
void EncryptionHelper::setMic(EapolFrame& eapol, const uint8_t* ptk, const int akmSuite)
{
...
void EncryptionHelper::decryptGtk(const uint8_t* ptk, const uint8_t suite, const uint8_t* encryptedGtk, const size_t encryptedLen,
						uint8_t* decryptedGtk)
{
	const int kekLen = getKekLength(suite);
...
int EncryptionHelper::getKekLength(const uint8_t suite)
{
	switch (suite)
	{
		case AKM_SUITE_PSK:
...
void EncryptionHelper::printPacketDebug(const u_char* packet, const uint32_t length)
{
